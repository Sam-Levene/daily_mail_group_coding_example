...
import com.dmgmedia.daily_mail_group_coding_example.reporting.ReportGenerator;
import com.dmgmedia.daily_mail_group_coding_example.utils.HtmlTags;
...
public class BrowserActions implements BrowserActionsInterface {
    ...
    public String getColor() {
        return Color.fromString(focusedElement.getCssValue("background-color")).asHex();
    }
    ...
    public BrowserActions changeUrl(String url) {
        driver.get(url);
        return this;
    }

    public BrowserActions scrollToElement(By by) throws BrowserException {
        if (execute()) {
            ...
            try {
                ...
                logger.error("Element wasn't found in the DOM: " + by.toString());
    ...
    public BrowserActionsInterface collect(By by) throws BrowserException {
        if (execute()) {
            ...
            if (focusedList.isEmpty()) {
                try {
                    throw new BrowserException("BrowserActions : collect() : no collection with collector [ " + by.toString() + " ]");
                } catch (Exception e) {
    ...
    public BrowserActionsInterface present(String text) throws BrowserException {
        if (execute()) {
            ...
            if (focusedList.isEmpty()) {
                ...
                for (WebElement element : focusedList) {
                    if (element.getText().isEmpty() && element.getAttribute(PLACEHOLDER).isEmpty() && element.getAttribute(VALUE).isEmpty()) {
                        try {
                            throw new BrowserException("*** BrowserActions: present(): Element has no text that can be present");
                        } catch (Exception e) {
                            logger.warn(e.getMessage());
                        }
                    } else if (element.getText().isEmpty() && element.getAttribute(PLACEHOLDER).isEmpty() && !element.getAttribute(VALUE).isEmpty()) {
                        if (!element.getAttribute(VALUE).equals(text)) {
                            try {
                                throw new BrowserException(PRESENT_ELEMENT_TEXT_OF + element.getAttribute(VALUE) + NO_MATCH_EXPECTED_TEXT + text + "\"");
                            } catch (Exception e) {
                                logger.warn(e.getMessage());
                            }
                        } else {
                        ...
                        if (!element.getAttribute(PLACEHOLDER).equals(text)) {
                            try {
                                throw new BrowserException(PRESENT_ELEMENT_TEXT_OF + element.getAttribute(PLACEHOLDER) + NO_MATCH_EXPECTED_TEXT + text + "\"");
                            } catch (Exception e) {
                                logger.warn(e.getMessage());
                            }
                        } else {
            ...
            if (focusedList.isEmpty()) {
                ...
                for (WebElement element : focusedList) {
                    if (element.getText().isEmpty() && element.getAttribute(PLACEHOLDER).isEmpty() && element.getAttribute(VALUE).isEmpty()) {
                        ...
                        if (element.getAttribute(VALUE).equals(text)) {
                            try {
                                throw new BrowserException(ABSENT_ELEMENT_TEXT_OF + element.getAttribute(VALUE) + MATCH_EXPECTED_TEXT + text + NOT_EXPECTED_TO);
                            } catch (Exception e) {
                                logger.warn(e.getMessage());
                            }
                        } else {
                        ...
                        if (element.getAttribute(PLACEHOLDER).equals(text)) {
                            try {
                                throw new BrowserException(ABSENT_ELEMENT_TEXT_OF + element.getAttribute(PLACEHOLDER) + MATCH_EXPECTED_TEXT + text + NOT_EXPECTED_TO);
                            } catch (Exception e) {
                                logger.warn(e.getMessage());
                            }
                        } else {
    ...
    public BrowserActionsInterface store() throws BrowserException {
        if (execute()) {
            ...
            try {
                if (isNullOrEmpty(focusedElement.getText()) && isNullOrEmpty(focusedElement.getAttribute(PLACEHOLDER)) && isNullOrEmpty(focusedElement.getAttribute(VALUE))) {
                    throw new BrowserException("The current focused element \"" + focusedElement.toString() + "\" has no text to store");
                } else if (isNullOrEmpty(focusedElement.getText()) && isNullOrEmpty(focusedElement.getAttribute(PLACEHOLDER))) {
                    storedText = focusedElement.getAttribute(VALUE);
                } else if (isNullOrEmpty(focusedElement.getText()) && isNullOrEmpty(focusedElement.getAttribute(VALUE))) {
                    storedText = focusedElement.getAttribute(PLACEHOLDER);
                } else {
        ...
        if (execute()) {
            ...
            try {
                if (isNullOrEmpty(key)) {
                   ...
                   textMap.put(key,focusedElement.getAttribute(VALUE));
                } else if (isNullOrEmpty(focusedElement.getText()) && isNullOrEmpty(focusedElement.getAttribute(VALUE))) {
                    textMap.put(key,focusedElement.getAttribute(PLACEHOLDER));
                } else {
        ...
        if (execute()) {
            ...
            try {
                if (isNullOrEmpty(focusedElement.getText()) && isNullOrEmpty(focusedElement.getAttribute(PLACEHOLDER)) && isNullOrEmpty(focusedElement.getAttribute(VALUE))) {
                   ...
                   if (!activeText.equals(focusedElement.getAttribute(VALUE))) {
                       report.setStatusLine(COMPARE_NO_MATCH + activeText + VERSUS + focusedElement.getAttribute(VALUE));
                       report.setActionInfo(NO_MATCH + activeText + VERSUS + focusedElement.getAttribute(VALUE));
                       throw new BrowserException(ACTIVE_TEXT + activeText + DID_NOT_COMPARE + focusedElement.getAttribute(VALUE) + "\"");
                   }
                } else if (isNullOrEmpty(focusedElement.getText()) && isNullOrEmpty(focusedElement.getAttribute(VALUE))) {
                    if (!activeText.equals(focusedElement.getAttribute(PLACEHOLDER))) {
                        report.setStatusLine(COMPARE_NO_MATCH + activeText + VERSUS + focusedElement.getAttribute(PLACEHOLDER));
                        report.setActionInfo(NO_MATCH + activeText + VERSUS + focusedElement.getAttribute(PLACEHOLDER));
                        throw new BrowserException(ACTIVE_TEXT + activeText + DID_NOT_COMPARE + focusedElement.getAttribute(PLACEHOLDER) + "\"");
                    }
                } else {
    ...
    public BrowserActionsInterface pause(Integer seconds) {
        ...
        try {
            ...
            report.setActionInfo(seconds.toString() + " second(s)");
    ...
    public String getText() throws BrowserException {
        ...
        String focusedElementText = "";
            ...
            if (focusedElement.getText().isEmpty() && focusedElement.getAttribute(PLACEHOLDER).isEmpty() && focusedElement.getAttribute(VALUE).isEmpty()) {
                logger.error("focusedElement had no text to compare against");
            } else if (focusedElement.getText().isEmpty() && focusedElement.getAttribute(PLACEHOLDER).isEmpty() && !focusedElement.getAttribute(VALUE).isEmpty()) {
                focusedElementText = focusedElement.getAttribute(VALUE);
            } else if (focusedElement.getText().isEmpty() && !focusedElement.getAttribute(PLACEHOLDER).isEmpty() && focusedElement.getAttribute(VALUE).isEmpty()) {
                focusedElementText = focusedElement.getAttribute(PLACEHOLDER);
            } else {
                focusedElementText = focusedElement.getText();
            }

            if (focusedElementText.equals("")) {
                try {
    ...
    private void hasSpecificContentValidator(WebElement element, String content) throws BrowserException {
        ...
        if (element.getText().isEmpty()) {
            if (element.getAttribute(PLACEHOLDER).isEmpty()) {
    ...
    private void hasContentValidator(WebElement element) throws BrowserException {
        ...
        if (element.getText().isEmpty() && element.getAttribute(PLACEHOLDER).isEmpty()) {
            try {
    ...
    private String getElementInfo(WebElement webElement) {
        ...
        if (!elementText.isEmpty()) {
            ...
            elementText = webElement.getAttribute(PLACEHOLDER);
